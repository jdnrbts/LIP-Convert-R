...
# LIP Convert-R (ANPC LIP --> LIPIDOME-R) 
...
ANPCdf_long <- sorted_df %>% 
  pivot_longer(cols = 10:ncol(sorted_df), names_to = "Name", values_to = "value")

ANPCdf_long$Name <- as.factor(ANPCdf_long$Name)
ANPCdf_long$value <- as.numeric(ANPCdf_long$value)
...
ANPCdf_long$'Concentration_plus1' <- ANPCdf_long$value + 1

ANPCdf_long$'Concentration_log10_umol_per_mL' <-
   log10(ANPCdf_long$Concentration_plus1)

names.mapping <- map_lipid_names(x = unique(ANPCdf_long$Name))
...
heatmap_lipidome(
   x = ANPCdf_long[ , c( "Name", "Concentration_log10_umol_per_mL" ) ],
   names.mapping = names.mapping,
   class.facet = "wrap",
   x.names = "Name",
   fill.limits =
       range(
           x = ANPCdf_long$"Concentration_log10_umol_per_mL",
           na.rm = TRUE
       ),
   fill.midpoint =
       sum(
...
# Convert the data into wide format, where each lipid is one column and each sample is one row and ompute the regression models.

ANPCdf_wide <-
   tidyr::pivot_wider(
       data = ANPCdf_long[,-c(12,13)],
       names_from = "Name",
       values_from = "value"
   )


ANPCdf_wide$sample_type <- factor(ANPCdf_wide$sample_type, levels = c("sample", "qc"))


names.mapping <-
   map_lipid_names( x = unique(ANPCdf_long$"Name"))
